...
name: implement-task
description: >-
  Dispatch and implement one tracked TODO.md CODE end-to-end on its own branch,
  then drive it through the full github-skill PR lifecycle (branch -> PR ->
  squash-merge -> tracking + board sync). Never lands work as a manual commit on main.
---

# Implement Task (`/implement-task <CODE> [project-dir]`)

Dispatch a subagent to implement one tracked `TODO.md` `CODE` end-to-end on its own branch, then drive it through the full `github` skill PR lifecycle (branch → PR → squash-merge → tracking + board sync). Never lands work as a manual commit on `main`.
...
- `project-dir` — repo-relative directory holding `TODO.md`/`instruction.md`/`docs/`. Default:
  `.` (this repo's root — YixinBoard keeps its tracking files there, see `GEMINI.md`/`CLAUDE.md`) if omitted.
...
procedure automates that lifecycle so it actually happens.

**Anti-pattern this workflow prevents (Sprint 7):** ENG-02, UI-08, UI-09, and REL-02 were
...
not do this. If a `CODE` is scoped enough to implement, it goes through this workflow (or, done by
...
- The `Sprint <N>` milestone exists (`gh api repos/:owner/:repo/milestones --jq '.[].title'`).
  Create it if not: `gh api repos/:owner/:repo/milestones -f title="Sprint <N>" -f state=open -f description="<sprint goal from docs/sprint/current.md>"`.
...
Subagent type: `general-purpose` unless the todo file names a more specific one; model per the
`[Model: <tier>]` tag or `AGENTS.md` default (Sonnet 5). Use an isolated worktree branch so it works on
...
- **Git discipline for the agent:**
  ...
  - Commit in logical chunks; every message `<CODE>: <imperative summary>`, keeping the
    `Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>` or appropriate model trailer. Intermediate messages are
    unconstrained but must still start `<CODE>:`.
  - Write the regression test (`GEMINI.md`/`CLAUDE.md` bug-fix rule) — never discard it.
  - Update the tracking files **in the same branch**, per `.agents/rules/tracking-files.md`: flip
...
- Assumes the `CODE` already went through discussion→design→todo formalization (`GEMINI.md`/`CLAUDE.md`). A
  `CODE` still sitting in `features/<slug>/planning.md` with open questions → resolve planning
  first; this workflow is the wrong tool.
