...
- ✅ **ANLZ-01.** Analyze Mode — continuous background analysis so WinGraph fills a real point for every position the user visits (the "Lizzie way"); no formula backfill on the plotted line. Orthogonal to "Engine plays". Design resolved — `features/analyze-mode/planning.md` Q1–Q8 accepted 2026-09-04. Supersedes the `GRAPH-xx` "evaluate the whole played line" idea. [Model: Sonnet 5] — implemented on branch `anlz-01/continuous-analyze-mode` 2026-09-04 (pending merge); build clean, ctest 3/3, +`test_anlz01_analyze_mode_coverage.cpp` / `test_anlz01_analyze_mode_action.cpp`; manual live smoke still needs a human — [detail](docs/todo/ANLZ-01-continuous-analyze-mode.md) · [instruction](docs/instruction/ANLZ-01-continuous-analyze-mode.md) · [fix-log](docs/fix-log/2026-09-04-analyze-mode.md) · design `features/analyze-mode/`
