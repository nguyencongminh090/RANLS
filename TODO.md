...
_(nothing committed to a sprint yet — see `docs/sprint/current.md`)_

## Backlog

Filed 2026-08-21 from a full read of `src/` (UI/UX + codebase review). Ordered by priority.
Prefixes: `RT` realtime pipeline · `STATE` state lifetime · `PROTO` engine protocol ·
`ENG` engine lifecycle · `NAV` navigation · `UI` display logic · `UX` usability · `TEST` harness ·
`CLEAN` hygiene.

### P0 — memory safety and incorrect data on screen

- **PROTO-01.** Harden the Gomocup parser: out-of-bounds `currentPVs_[-1]` in `onPVDone`, unbounded `NUMPV` resize, unvalidated database coords — [detail](docs/todo/PROTO-01-parser-hardening.md)
- **STATE-01.** Stale PV / engine status / board markers survive New Game, makeMove, and undo/redo — [detail](docs/todo/STATE-01-stale-analysis-after-position-change.md)
...
- **RT-01.** No throttle anywhere on the engine→UI analysis path; 6 emit sites drive a full UI rebuild per parsed line — [detail](docs/todo/RT-01-throttle-analysis-signal.md)
...
### P1 — wrong results, wasted work

- **TEST-01.** No test infrastructure exists — blocks the regression tests STATE-01 and PROTO-01 require — [detail](docs/todo/TEST-01-test-infrastructure.md)
