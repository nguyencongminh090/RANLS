...
// UX-04: this is the single definition of the coordinate-label margin.
// board_view.cpp previously kept its own copy of this constant (and the
// whole cell-size/margin formula) for BoardView::pixelToCoord() -- the two
// could silently drift apart if only one was edited. Both now go through
// BoardRenderer::computeGeometry() below.
