...
| NAME-01 | Consistent app-wide rename `"Rapfi Analysis" → RANLS` (window title, `style.css`, GTK app id, CMake) — split out of UI-11 | — | — | ✅ Done — merged to `main` in PR #8 (squash `0ec6cd2`) 2026-09-03 (full target rename `rapfi-gui` → `ranls-gui`; +`test_name01_window_title.cpp`; build + ctest 3/3 green) |
