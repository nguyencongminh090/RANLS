...
**Status:** ✅ DONE (Sprint 15, 2026-09-06) — all four scope items landed (item 4, the Windows CI job, deferred as follow-up per its "optional" clause). `CMakeLists.txt`: `if(MSVC) /W4 /permissive- else() -Wall -Wextra -Wpedantic`, `WIN32` on `add_executable(ranls-gui …)`, gtkmm/gio discovery gated `if(MSVC)`→`find_package(PkgConfig)` shim path (else = unchanged pkg-config) + a documented vcpkg route (README "Building on Windows", `build.sh`/`build_msys2.sh` pointers). New `tests/mock_engine/` — one source, two targets (`mock_engine` cat-like echo/stay-alive/exit-on-"END"/EOF, `mock_engine_quit` = `/bin/true`); their `$<TARGET_FILE>` paths passed to both test binaries as `MOCK_ENGINE_PATH` / `MOCK_ENGINE_QUIT_PATH` compile defs; all 7 suites migrated off `/bin/cat`+`/bin/true` (assertions untouched), `test_eng03_pdeathsig.cpp` left Linux-guarded. `src/engine/engine_process.cpp`: `#elif defined(_WIN32)` Win32 Job Object (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, process assigned right after spawn) — Linux `PR_SET_PDEATHSIG` path and the macOS/other `Gio::Subprocess` fallback both untouched (macOS gap documented in a comment). Also fixed a pre-existing UAF surfaced by the migration: `BottomPanel`'s deferred scroll idles captured a bare `this` and crashed in `Gtk::TextView::scroll_to` if the panel was destroyed first (the mock_engine path's longer engine-log line changed the scroll geometry enough to make it deterministic; same defect behind the "ui12 scroll" flake) — now `sigc::track_obj`-bound, + a `PORT-03:` regression case in `test_ui10_engine_log_scroll_target.cpp`.

**Verification (Linux tier, 2026-09-06, this host — no Windows toolchain / display-less CI):** clean `cmake -S . -B build && cmake --build build` from scratch — `ranls-gui`, `mock_engine`, `mock_engine_quit` all build/link, no new warnings (only the 3 pre-existing `-Wunused-function` in `gomocup_protocol.cpp`). `ctest`: `port02-style-css-bundled` ✅, `ranls-gui-tests` ✅ 209/209 cases (2466 assertions — includes the migrated ENG-01/ANLZ-05/ANLZ-06/PROTO-04 suites on `mock_engine`), `rel02-version-single-source` ✅, `ranls-gui-ui-tests` 29/30 cases — the lone failure `test_anlz05_no_automove_action` (ANLZ-05 wire race) reproduces identically on the pre-PORT-03 baseline (`670d0dc`), no crash, 0 skipped (was a deterministic SIGSEGV before the BottomPanel fix). No lingering `/bin/cat`/`/bin/true` in test code (comments only).

**Windows tier — pending a human MSVC/Windows smoke test:** native MSVC (or clang-cl) compile of `ranls-gui`, no console window on GUI launch, and `taskkill /f` on the GUI leaving no orphaned engine process. Not verifiable on this host.

**Follow-up:** Windows GitHub Actions job (scope item 4) — deferred; safe to add now that 1–3 are in.
